...
static connector_callback_status_t dp_handle_length_callback(connector_data_t * const connector_ptr, connector_data_service_length_t * const data_ptr)
{
    ...
    switch (dp_info->type)
    {
...
#endif
        {
            ...
            /*
             * ETHERIOS-EGEAR-MOD start
             */
            if (sm_ptr != NULL)		/* Needed in-case ASSERT() is deactivated */
            {
             ...
             * ETHERIOS-EGEAR-MOD:
             *
             * Should never occur, but force
             * Connector abort if it does.
             */
            else
            {
