        ...
        {   /* continue for any pending receive */
            ...
            switch (status)
            {
                ...
                case connector_callback_busy:
                    ...
                    goto done;
                case connector_callback_error:
                    result = connector_exceed_timeout;      /* Ikayzo E-Gear Mod */
                    goto done;
                case connector_callback_abort:
    ...
    if (result == connector_working)
    {
        ...
        */
        ...
        if (message_load_be16(edp_header, type) == E_MSG_MT2_TYPE_PAYLOAD)
        {
            ...
            {
                uint16_t const length = (total_length < PACKET_EDP_PROTOCOL_SIZE) ? 0 : (uint16_t)(total_length - PACKET_EDP_PROTOCOL_SIZE);
