...
static connector_callback_status_t app_get_ip_address(connector_config_ip_address_t * const config_ip)
{
...

    {
        ...
        APP_DEBUG("get_ip_address: Looking for current device IP address: found [%u] entries\n", entries);
...
{
/* #error "Specify device MAC address for LAN connection" */	/* ETHERIOS-EGEAR-MOD */
...
{
/* #error  "Specify vendor id" */ 	/* ETHERIOS-EGEAR-MOD */
 ...
 */
static char connector_cloud_phone[] = "447786201216"; /* phone number corresponding to devicecloud.digi.com, NOTE: Same from login.etherios.com */
...
static connector_callback_status_t app_set_device_cloud_phone(connector_config_pointer_string_t * const config_phone)
{
    ...
    (void)memset(connector_cloud_phone, 0, sizeof(connector_cloud_phone));

    (void)strncpy(connector_cloud_phone, config_phone->string, (sizeof(connector_cloud_phone) - 1));
...
{
    static char connector_password[] = "";

    /* Return pointer to password. */
    config_password->string = (char *)connector_password;
    config_password->length = strlen(connector_password);
...
static char const * app_network_class_to_string(connector_request_id_network_t const value)
{
    ...
    switch (value)
    {
        ...
        enum_to_case(connector_request_id_network_open_stub);
    }
    return result;
}
